...
def test_engine_threads_source_network_to_deep_read():
    ...
    # v2.1.2: engine calls read_object_list_count + read_object_list_entries
    # (not the convenience wrapper). Mock both.
    mock_client.read_object_list_count.return_value = 0
    mock_client.read_object_list_entries.return_value = []
    ...
    ol_count_kwargs = mock_client.read_object_list_count.call_args.kwargs
    assert ol_count_kwargs.get('dnet') == 103
    assert ol_count_kwargs.get('dadr') == '1'
